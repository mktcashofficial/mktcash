...
pkgdatadir = $(datadir)/mktcash
pkgincludedir = $(includedir)/mktcash
pkglibdir = $(libdir)/mktcash
pkglibexecdir = $(libexecdir)/mktcash
...
CONFIG_HEADER = $(top_builddir)/src/config/mktcash-config.h
...
am__DIST_COMMON = $(srcdir)/Makefile.in \
	...
	$(top_srcdir)/src/config/mktcash-config.h.in \
...
ACLOCAL = ${SHELL} /prj/coinFactory/works/bitcoin/mktcash/build-aux/missing aclocal-1.15
AMTAR = $${TAR-tar}
AM_DEFAULT_VERBOSITY = 0
AR = /usr/bin/ar
ARFLAGS = cr
AUTOCONF = ${SHELL} /prj/coinFactory/works/bitcoin/mktcash/build-aux/missing autoconf
AUTOHEADER = ${SHELL} /prj/coinFactory/works/bitcoin/mktcash/build-aux/missing autoheader
AUTOMAKE = ${SHELL} /prj/coinFactory/works/bitcoin/mktcash/build-aux/missing automake-1.15
AVX2_CXXFLAGS = -mavx -mavx2
AWK = gawk
BDB_CFLAGS = 
BDB_CPPFLAGS = 
BDB_LIBS = -ldb_cxx-4.8
BITCOIN_CLI_NAME = mktcash-cli
BITCOIN_DAEMON_NAME = mktcashd
BITCOIN_GUI_NAME = mktcash-qt
BITCOIN_TX_NAME = mktcash-tx
...
LUPDATE = 
MAINT = 
MAKEINFO = ${SHELL} /prj/coinFactory/works/bitcoin/mktcash/build-aux/missing makeinfo
MAKENSIS = 
MANIFEST_TOOL = :
MINIUPNPC_CPPFLAGS = 
...
OTOOL64 = 
PACKAGE = mktcash
PACKAGE_BUGREPORT = mktcash.io
PACKAGE_NAME = Mktcash Core
PACKAGE_STRING = Mktcash Core 1.0.0
PACKAGE_TARNAME = mktcash
PACKAGE_URL = 
...
ZMQ_CFLAGS = 
ZMQ_LIBS = -lzmq
abs_builddir = /prj/coinFactory/works/bitcoin/mktcash
abs_srcdir = /prj/coinFactory/works/bitcoin/mktcash
abs_top_builddir = /prj/coinFactory/works/bitcoin/mktcash
abs_top_srcdir = /prj/coinFactory/works/bitcoin/mktcash
ac_ct_AR = ar
ac_ct_CC = gcc
ac_ct_CXX = g++
ac_ct_DUMPBIN = 
...
install_sh = ${SHELL} /prj/coinFactory/works/bitcoin/mktcash/build-aux/install-sh
...
DIST_CONTRIB = $(top_srcdir)/contrib/mktcash-cli.bash-completion \
	       $(top_srcdir)/contrib/mktcash-tx.bash-completion \
	       $(top_srcdir)/contrib/mktcashd.bash-completion \
...
COVERAGE_INFO = baseline.info \
  test_mktcash_filtered.info total_coverage.info \
  baseline_filtered.info functional_test.info functional_test_filtered.info \
  test_mktcash_coverage.info test_mktcash.info

OSX_APP_BUILT = $(OSX_APP)/Contents/PkgInfo $(OSX_APP)/Contents/Resources/empty.lproj \
  $(OSX_APP)/Contents/Resources/bitcoin.icns $(OSX_APP)/Contents/Info.plist \
  $(OSX_APP)/Contents/MacOS/Mktcash-Core $(OSX_APP)/Contents/Resources/Base.lproj/InfoPlist.strings
...
src/config/mktcash-config.h: src/config/stamp-h1
	@test -f $@ || rm -f src/config/stamp-h1
	@test -f $@ || $(MAKE) $(AM_MAKEFLAGS) src/config/stamp-h1

src/config/stamp-h1: $(top_srcdir)/src/config/mktcash-config.h.in $(top_builddir)/config.status
	@rm -f src/config/stamp-h1
	cd $(top_builddir) && $(SHELL) ./config.status src/config/mktcash-config.h
$(top_srcdir)/src/config/mktcash-config.h.in:  $(am__configure_deps) 
	($(am__cd) $(top_srcdir) && $(AUTOHEADER))
	rm -f src/config/stamp-h1
	touch $@

distclean-hdr:
	-rm -f src/config/mktcash-config.h src/config/stamp-h1
...
$(OSX_APP)/Contents/MacOS/Mktcash-Core: all-recursive
	$(MKDIR_P) $(@D)
...
$(APP_DIST_EXTRAS): $(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/Mktcash-Core
...
$(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/Mktcash-Core: $(OSX_APP_BUILT) $(OSX_PACKAGING)
...
#test_mktcash.info: baseline_filtered.info
#	$(MAKE) -C src/ check
#	$(LCOV) -c $(LCOV_OPTS) -d $(abs_builddir)/src -t test_mktcash -o $@
#	$(LCOV) -z $(LCOV_OPTS) -d $(abs_builddir)/src

#test_mktcash_filtered.info: test_mktcash.info
#	$(abs_builddir)/contrib/filter-lcov.py $(LCOV_FILTER_PATTERN) $< $@
#	$(LCOV) -a $@ $(LCOV_OPTS) -o $@

#functional_test.info: test_mktcash_filtered.info
...
#test_mktcash_coverage.info: baseline_filtered.info test_mktcash_filtered.info
#	$(LCOV) -a $(LCOV_OPTS) baseline_filtered.info -a test_mktcash_filtered.info -o $@

#total_coverage.info: test_mktcash_filtered.info functional_test_filtered.info
#	$(LCOV) -a $(LCOV_OPTS) baseline_filtered.info -a test_mktcash_filtered.info -a functional_test_filtered.info -o $@ | $(GREP) "\%" | $(AWK) '{ print substr($$3,2,50) "/" $$5 }' > coverage_percent.txt

#test_mktcash.coverage/.dirstamp:  test_mktcash_coverage.info
...
#cov: test_mktcash.coverage/.dirstamp total.coverage/.dirstamp
...
clean-local: clean-docs
	rm -rf coverage_percent.txt test_mktcash.coverage/ total.coverage/ test/tmp/ cache/ $(OSX_APP)
