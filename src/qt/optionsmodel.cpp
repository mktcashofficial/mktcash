...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::MCH);
    ...
    if (!settings.contains("nAnonymizeMCHAmount"))
        settings.setValue("nAnonymizeMCHAmount", 1000);

    nAnonymizeMCHAmount = settings.value("nAnonymizeMCHAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeMCHAmount"))
        SoftSetArg("-anonymizemktcashamount", settings.value("nAnonymizeMCHAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in mktcash.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeMCHAmount:
            return QVariant(nAnonymizeMCHAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case HideOrphans:
            ...
            break;
        case AnonymizeMCHAmount:
            nAnonymizeMCHAmount = value.toInt();
            settings.setValue("nAnonymizeMCHAmount", nAnonymizeMCHAmount);
            emit anonymizeMCHAmountChanged(nAnonymizeMCHAmount);
            break;
        case CoinControlFeatures:
