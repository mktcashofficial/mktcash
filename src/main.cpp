...
#if defined(NDEBUG)
#error "MCH cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in umktcash) are considered zero fee (for relaying and mining)
...
int64_t GetBlockValue(int nHeight)
{
   ...
   } else if (nHeight <= 249999 && nHeight >= 201) {
          nSubsidy = 372 * COIN;
   } else if (nHeight <= 274999 && nHeight >= 250000) {
          nSubsidy = 180 * COIN;
   } else if (nHeight <= 299999 && nHeight >= 275000) {
          nSubsidy = 80 * COIN;
   } else if (nHeight <= 324999 && nHeight >= 300000) {
          nSubsidy = 40 * COIN;
   } else if (nHeight <= 349999 && nHeight >= 325000) {
        nSubsidy = 20 * COIN;
    } else {
        nSubsidy = 10 * COIN;
    }
...
{
    RenameThread("mktcash-scriptch");
    scriptcheckqueue.Thread();
}

bool RecalculateMCHSupply(int nHeightStart)
{
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // MCH: We use certain sporks during IBD, so check to see if they are
